                ...
                <div class="w-full">
                    ...
                    @if( !$setPassword )
                        <div
                            ...
                            <form wire:submit.prevent="resetPassword" x-show="!isVerifyOpen()">
                                @csrf
                                <x-input title="{{__('Phone')}}" placeholder="019********" name="phone" />
                                <div x-show="!loading">
                                    <x-buttons.primary title="{{__('Reset Password')}}" id="reset-btn" />
                                </div>
                    ...
                    @else
                        <form wire:submit.prevent="saveNewPassword">
                            @csrf
                            <x-input title="{{__('New Password')}}" type="password" placeholder="***************" name="password" />
                            <x-input title="{{__('Confirm New Password')}}" type="password" placeholder="***************" name="password_confirmation" />
